...
const plugin = {
    id: 'toolUseRevisionCornerWidgetsToggle',
    name: 'Tool Use Revision Corner Widgets Toggle',
    description:
    ...
    onMutation(state) {
        ...
        if (!slot) {
            if (!state.missingLogged) {
                Logger.debug('Tool Use Revision Corner Widgets Toggle: top bar actions container not found');
...
`;
        ...
        state.styleInjected = true;
        Logger.log('Tool Use Revision Corner Widgets Toggle: corner widgets hide style injected');
    },
    ...
    setCornerWidgetsHidden(hidden, persist) {
        ...
        if (persist) {
            ...
            Logger.log(
                `Tool Use Revision Corner Widgets Toggle: corner widgets ${hidden ? 'hidden' : 'shown'} (CSS only)`
            );
        }
    ...
    ensureToggleWrapper(slot, state) {
        ...
        if (!Storage.getSubOptionEnabled(this.id, CORNER_WIDGETS_SUBOPTION_ID, true)) {
            ...
            if (wrapper) {
                wrapper.remove();
                Logger.log(
                    'Tool Use Revision Corner Widgets Toggle: toggle wrapper removed (subOption disabled)'
                );
            }
            this.setCornerWidgetsHidden(false, false);
            return;
        ...
        if (!wrapper) {
            ...
            if (!state.toggleLogged) {
                Logger.log('Tool Use Revision Corner Widgets Toggle: toggle added to top bar');
        ...
        if (!toggleBtn) {
            ...
            Logger.log('Tool Use Revision Corner Widgets Toggle: corner widgets toggle button added');
        }
