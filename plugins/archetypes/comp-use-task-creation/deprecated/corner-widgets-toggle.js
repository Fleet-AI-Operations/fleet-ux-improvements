...
const plugin = {
    id: 'compUseTaskCreationCornerWidgetsToggle',
    name: 'Computer Use Task Creation Corner Widgets Toggle',
    description:
    ...
    onMutation(state) {
        ...
        if (!slot) {
            if (!state.missingLogged) {
                Logger.debug('Computer Use Task Creation Corner Widgets Toggle: top bar actions container not found');
...
`;
        ...
        state.styleInjected = true;
        Logger.log('Computer Use Task Creation Corner Widgets Toggle: corner widgets hide style injected');
    },
    ...
    setCornerWidgetsHidden(hidden, persist) {
        ...
        if (persist) {
            ...
            Logger.log(
                `Computer Use Task Creation Corner Widgets Toggle: corner widgets ${hidden ? 'hidden' : 'shown'} (CSS only)`
            );
        }
    ...
    ensureToggleWrapper(slot, state) {
        ...
        if (!Storage.getSubOptionEnabled(this.id, CORNER_WIDGETS_SUBOPTION_ID, true)) {
            ...
            if (wrapper) {
                wrapper.remove();
                Logger.log(
                    'Computer Use Task Creation Corner Widgets Toggle: toggle wrapper removed (subOption disabled)'
                );
            }
            this.setCornerWidgetsHidden(false, false);
            return;
        ...
        if (!wrapper) {
            ...
            if (!state.toggleLogged) {
                Logger.log('Computer Use Task Creation Corner Widgets Toggle: toggle added to top bar');
        ...
        if (!toggleBtn) {
            ...
            Logger.log('Computer Use Task Creation Corner Widgets Toggle: corner widgets toggle button added');
        }
