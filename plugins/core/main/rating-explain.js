...
function buildRatingExplainAbout() {
    ...
    return [
        ...
        '',
        'Each card computes **two variants** of every score simultaneously. The toolbar **Recency / Flat** toggle applies to all cards:',
        '',
        ...
        '',
        'JSON export always includes **both** weighting variants. The toggle only changes what is displayed. The payload\'s `weighting` field identifies the displayed variant.',
        '',
        '## Population tier',
        '',
        'The **primary display** is a population tier label (Poor, Below average, Typical, Above average, Top tier), with an **estimated percentile** shown beside it (for example, ~62nd). The headline also shows the previous-population overall percentile in parentheses (for example, ~37th (old: ~61st)) — the pre-dismissal population mapped in current score units. Team, environment, and month subset rows show the current percentile only.',
        '',
        ...
        '- The **Recency / Flat** toggle applies the displayed weighting to all cards.',
        '- The **Time / No Time** toggle includes or excludes tracked-time axes from the composite (axes still display when off).',
        ...
        '- QAQS also excludes Flag as Bug QA rows created when the same person resolved a dispute on that task.',
        '',
        '## The axes',
        '',
        ...
        '| --- | ---: | --- |',
        '| Outcome Quality | 30% | Blend of current terminal quality and flat closure quality: production 1.0, discarded 0.5, dismissed 0.0. Closure excludes bugged/flagged paths. |',
        '| V1 Creation Time | 20% | Each timed task is scored against that project\'s normal time band (middle half of population times for the env, else team, else global). Full credit inside the band; the score falls toward zero below the low edge or above the long-tail fence. Toggleable. |',
        '| Positive Feedback Rate | 15% | Share of human QA on their tasks that was an accept. Prompt-quality labels are ignored. Self-reviews excluded. |',
        '| Edits by QA | 10% | Share of terminal tasks with version history that QA did not patch. A patch is a non-latest display version with no linked feedback. v1 counts as clean. |',
        '| Task Rating Quality | 10% | Mean of peer prompt-quality labels on their tasks: Bottom 10% = 0, Average = 0.5, Top 10% = 1. Missing labels count as Average. |',
        '| Revision Efficiency | 10% | On terminal tasks with version history: full credit at one display version; each extra version halves credit (0.5^(v−1)). Upheld writer disputes do not count against the version total. QA-edit versions still count. |',
        '| Dispute Loss Avoidance | 5% | Rejected writer disputes as a rate vs tasks they authored. Full credit at zero losses; the score falls toward zero as losses approach 10% of tasks. Approved disputes are neutral. |',
        '',
        '### QA Quality Score (QAQS)',
        '',
        '| Axis | Weight | What it measures |',
        '| --- | ---: | --- |',
        '| Return Effectiveness | 30% | Of ordinary returns (not escalations or Flag-as-Bug) on tasks that stayed on a shippable path (production, bugged, or escalated), how often the task reached production. Discarded and dismissed tasks are excluded. |',
        '| QA Time | 20% | Each timed review is scored against that project\'s normal QA time band (middle half of population times for the env, else team, else global). Full credit inside the band; the score falls toward zero below the low edge or above the long-tail fence. Toggleable. |',
        '| Return Actionability | 20% | The task author responds positively to their return (the next human feedback is positive). |',
        '| Label Discrimination | 10% | Top and Bottom label usage each vs an ideal 10% rate (full credit at 10% per label; each side falls toward 0 at 0% and at 20%+). Axis is the average of the two. Omitted when fewer than 10 feedback rows are in scope. |',
        '| Acceptance Scrutiny | 10% | Two-sided check against unusually high or low accept rates. Full credit from 40% to 60%; the score falls toward zero as accepts approach 0% or 100%. |',
        '| Dispute Loss Avoidance | 10% | Dispute losses linked to this reviewer\'s feedback, as a rate vs tasks they QA\'d. Full credit at zero losses; the score falls toward zero as losses approach 10% of tasks. QA wins are neutral. |',
        '',
        '## Additional model-only payload guidance',
        '',
        '- Headline `tier`, `estimatedPercentile`, `estimatedPercentilePrevious`, and `score` match the card (main composite; not a cohort blend). `estimatedPercentilePrevious` is the overall percentile from the frozen previous population; omit it from team/environment/month slice discussion.',
        ...
        '- `excluded: true` on a tracked-time axis means Time/No Time is off; the axis score is shown but not in the composite.',
