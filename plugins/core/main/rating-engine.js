...
// Acceptance Scrutiny: one-sided penalty when shrunk accept rate exceeds this threshold.
const RE_QAQS_ACCEPT_SCRUTINY_T = 0.65;
...
// Snapshot: 1030 WPS / 285 QPS; flat + recency30d (engine 12.1 LD tent map).
...
const RE_TIER_THRESHOLDS = {
    ...
    qaqs: {
        flat:    { p10: 58.896, p30: 64.998, p70: 72.496, topMin: 72.506 },
        recency: { p10: 58.812, p30: 65.994, p70: 73.156, topMin: 73.166 },
    },
};
...
const RE_PERCENTILE_PARAMS = {
    ...
    qaqs: {
        flat:    { mu: 68.6038, sigma: 7.6875 },
        recency: { mu: 68.8485, sigma: 6.6916 },
    },
};
...
/** One-sided Acceptance Scrutiny score from a shrunk accept rate. */
function reAcceptanceScrutinyScore(acceptRate) {
    const a = Number(acceptRate);
    if (!Number.isFinite(a)) return null;
    const t = RE_QAQS_ACCEPT_SCRUTINY_T;
    if (a <= t) return 1;
    if (t >= 1) return 0;
    return Math.max(0, 1 - (a - t) / (1 - t));
}
