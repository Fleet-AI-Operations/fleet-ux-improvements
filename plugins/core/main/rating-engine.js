...
// Engine v19.0: TWQS Edits by QA axis (10%); OQ 30%; TW DLA 5%.

const RE_VERSION = '19.0';
...
// Snapshot: 388 WPS / 186 QPS; remaining population after 2026-08-24 mass dismissal,
// recalibrated 2026-08-25 for engine 19.0 (Edits by QA). Frozen previous percentile is separate.
...
const RE_TIER_THRESHOLDS = {
    twqs: {
        flat:         { p10: 68.604, p30: 72.631, p70: 77.909, topMin: 80 },
        recency:      { p10: 71.103, p30: 74.291, p70: 76.699, topMin: 80 },
        flatNoTime:   { p10: 69.051, p30: 73.452, p70: 78.388, topMin: 80 },
        recencyNoTime:{ p10: 71.411, p30: 74.561, p70: 77.0, topMin: 80 },
    },
    qaqs: {
        flat:         { p10: 70.155, p30: 74.075, p70: 78.05, topMin: 78.06 },
        recency:      { p10: 72.475, p30: 74.88, p70: 78.83, topMin: 78.84 },
        flatNoTime:   { p10: 70.615, p30: 73.96, p70: 78.105, topMin: 78.115 },
        recencyNoTime:{ p10: 71.905, p30: 75.02, p70: 79.275, topMin: 79.285 },
    },
};
...
// Snapshot: 388 WPS / 186 QPS; remaining population after 2026-08-24 mass dismissal,
// recalibrated 2026-08-25 for engine 19.0 (Edits by QA).
const RE_PERCENTILE_PARAMS = {
    twqs: {
        flat:         { mu: 74.9315, sigma: 5.0426 },
        recency:      { mu: 75.3581, sigma: 3.0936 },
        flatNoTime:   { mu: 75.3748, sigma: 5.2063 },
        recencyNoTime:{ mu: 75.5903, sigma: 3.3133 },
    },
    qaqs: {
        flat:         { mu: 75.4403, sigma: 5.0209 },
        recency:      { mu: 76.4963, sigma: 4.2428 },
        flatNoTime:   { mu: 75.7616, sigma: 5.1774 },
        recencyNoTime:{ mu: 76.9699, sigma: 4.5959 },
    },
};
...
const RE_PERCENTILE_PARAMS_PREVIOUS = {
    ...
    qaqs: {
        ...
        recency:      { mu: 76.593, sigma: 4.3061 },
        flatNoTime:   { mu: 76.0985, sigma: 5.7561 },
        recencyNoTime:{ mu: 77.1584, sigma: 4.7921 },
    },
};
...
/** Same detector as dashboard-lib `_taskHasQaEditedVersion` (QA patched a non-latest display version). */
function reTaskHasQaEditedVersion(task) {
    const lib = reLib();
    if (lib && typeof lib._taskHasQaEditedVersion === 'function') {
        return !!lib._taskHasQaEditedVersion(task);
    }
    ...
    for (const entry of (task && task.allFeedback) || []) {
        ...
        linked.add(Number(displayNo));
    }
    for (const version of versions) {
        if (version.displayVersionNo >= maxDisplayVersionNo) continue;
        if (!linked.has(Number(version.displayVersionNo))) return true;
    }
    return false;
}
...
/** Resolve peer prompt-quality label. Blank / missing = Average (no unscored state). */
function rePeerRatingLabel(rawRating, scoreFallback) {
    const raw = String(rawRating || '').trim();
    const scored = String(scoreFallback || '').trim();
    if (raw === 'Top 10%' || scored === 'Top 10%') return 'Top 10%';
    if (raw === 'Bottom 10%' || scored === 'Bottom 10%') return 'Bottom 10%';
    if (raw === 'Average' || scored === 'Average') return 'Average';
    return 'Average';
}

function reTaskRatingQualityValue(rawRating, scoreFallback) {
    const label = rePeerRatingLabel(rawRating, scoreFallback);
    return RE_TASK_RATING_QUALITY_VALUE[label];
}
...
function reValidateRolePriors(kind, priors) {
    ...
    const required = kind === 'twqs'
        ? ['outcomeQuality', 'positiveFeedbackRate', 'editsByQa', 'taskRatingQuality', 'revisionEfficiency', 'disputeLossAvoidance']
...
function reScoreBlockExport(block) {
    ...
    return {
        ...
        estimatedPercentilePrevious: block.estimatedPercentilePrevious != null
...
function reScoreBlockForLlm(mainBlock, kind, cohortEntry) {
    ...
    const out = {
        ...
        estimatedPercentilePrevious: mainBlock.estimatedPercentilePrevious != null
            ? mainBlock.estimatedPercentilePrevious
            : null,
...
const RatingEngine = {
    ...
    /** Frozen pre-mass-dismissal overall percentile for the same current score. */
    estimatePercentilePrevious(score, kind, weighting, includeTime) {
        const key = reCalibrationKey(weighting, includeTime);
        const params = (RE_PERCENTILE_PARAMS_PREVIOUS[kind] && RE_PERCENTILE_PARAMS_PREVIOUS[kind][key]) || null;
        return reEstimatePercentile(score, params);
    },
    ...
    // Computes WPS-aligned TWQS (seven-axis, rank-workers.py formula).
    _computeTwqs(workerId, hydratedItems, weighting, window, nowMs, cohort, includeTime, baselines) {
        ...
        // Task Rating Quality: every peer review, ordinal 0/0.5/1 (blank = Average)
        ...
        // Edits by QA: terminal tasks with version history; 1 if QA did not patch.
        let editsKsum = 0, editsNsum = 0;
        let editsTaskCount = 0, editsEditedCount = 0, editsCleanCount = 0;
        ...
        for (const item of writerItems) {
            ...
            if (terminalQuality !== null && taskInScope) {
                ...
                if (displayCount > 0 && taskW > 0) {
                    ...
                    // Edits by QA: same eligible set as RE; v1 is clean; QA-edit versions still count in RE.
                    const clean = reTaskHasQaEditedVersion(task) ? 0 : 1;
                    editsKsum += clean * taskW;
                    editsNsum += taskW;
                    editsTaskCount += 1;
                    if (clean) editsCleanCount += 1;
                    else editsEditedCount += 1;
                }
            }
            ...
            for (const entry of allHumanFb) {
                ...
                // Task Rating Quality — blank/missing label counts as Average
                const rawRating = entry.display && entry.display.qualityRatingRaw;
                const label = rePeerRatingLabel(rawRating, entry.score);
                const ratingValue = RE_TASK_RATING_QUALITY_VALUE[label];
                trqNsum += fw;
        ...
        const revEffScore = revEffDefined
            ...
            : null;
        const editsScore = reShrunkRate(editsKsum, editsNsum, RE_TWQS_C.editsByQa, priors.editsByQa);
        ...
        const axes = RE_TWQS_AXES.map((def) => {
            ...
            switch (def.id) {
                ...
                case 'editsByQa':
                    score = editsScore;
                    raw = {
                        weightedClean: Math.round(editsKsum * 1000) / 1000,
                        weightedN: Math.round(editsNsum * 1000) / 1000,
                        observedCounts: {
                            eligible: editsTaskCount,
                            qaEdited: editsEditedCount,
                            clean: editsCleanCount,
                        },
                    };
                    break;
                case 'revisionEfficiency':
        ...
        const prevTwqs = (RE_PERCENTILE_PARAMS_PREVIOUS.twqs && RE_PERCENTILE_PARAMS_PREVIOUS.twqs[calKey]) || {};
        combined.estimatedPercentilePrevious = reEstimatePercentile(combined.score, prevTwqs);
        ...
        combined.raw = {
            ...
            editsKsum: Math.round(editsKsum * 1000) / 1000,
            editsNsum: Math.round(editsNsum * 1000) / 1000,
    ...
    _computeQaqs(workerId, hydratedItems, weighting, window, nowMs, cohort, includeTime, baselines) {
        ...
        combined.estimatedPercentile = reEstimatePercentile(combined.score, params);
        const prevQaqs = (RE_PERCENTILE_PARAMS_PREVIOUS.qaqs && RE_PERCENTILE_PARAMS_PREVIOUS.qaqs[calKey]) || {};
        combined.estimatedPercentilePrevious = reEstimatePercentile(combined.score, prevQaqs);
    ...
    serializeMarkdown(workerReport) {
        ...
        const renderScoreBlock = (title, block) => {
            ...
            if (block.estimatedPercentile != null) {
                ...
                if (block.estimatedPercentilePrevious != null) {
                    scoreStr += ' (old: ~' + reFormatPercentile(block.estimatedPercentilePrevious) + ')';
                }
            }
