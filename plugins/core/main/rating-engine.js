...
// Engine v12.4: Dispute Loss Avoidance loss-rate curve (L/(T+C), zero at 10%).

const RE_VERSION = '12.4';
...
const RE_QAQS_DISPUTE_DEF_C     = 10;
...
// Dispute Loss Avoidance: shrunk loss rate L/(T+C); score 0 when rate reaches R0.
const RE_QAQS_DISPUTE_DEF_R0 = 0.10;
...
// Snapshot: 1030 WPS / 285 QPS; flat + recency30d (engine 12.4 DD loss-rate curve).
...
const RE_TIER_THRESHOLDS = {
    twqs: {
        flat:    { p10: 55.935, p30: 63.31, p70: 71.53, topMin: 80 },
        recency: { p10: 62.048, p30: 66.21, p70: 69.962, topMin: 80 },
    },
    qaqs: {
...
const RE_PERCENTILE_PARAMS = {
    twqs: {
        flat:    { mu: 66.6519, sigma: 8.2352 },
        recency: { mu: 67.9551, sigma: 5.0347 },
    },
    qaqs: {
...
/** Dispute Loss Avoidance: one-sided loss-rate curve. r = L/(T+C); score 1 at r=0, 0 at r≥R0. */
function reDisputeDefenseScore(losses, tasksReviewed) {
    const L = Number(losses);
    const T = Number(tasksReviewed);
    if (!Number.isFinite(L) || !Number.isFinite(T) || L < 0 || T < 0) return null;
    const C = RE_QAQS_DISPUTE_DEF_C;
    const R0 = RE_QAQS_DISPUTE_DEF_R0;
    const r = L / (T + C);
    if (!(R0 > 0)) return 0;
...
const RatingEngine = {
    ...
    _computeQaqs(workerId, hydratedItems, weighting, window, nowMs, cohort) {
        ...
        const ddLossRate = ddLosses / (ddTasksReviewed + RE_QAQS_DISPUTE_DEF_C);
        const ddScore = reDisputeDefenseScore(ddLosses, ddTasksReviewed);
            ...
            switch (def.id) {
                ...
                case 'disputeDefense':
                    ...
                    raw = {
                        ...
                        threshold: RE_QAQS_DISPUTE_DEF_R0,
                        observedCounts: {
                            losses: ddLosses,
                            wins: ddWins,
                            resolved: ddDenom,
