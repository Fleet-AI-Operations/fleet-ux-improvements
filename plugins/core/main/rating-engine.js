...
// Engine v13.0: V1 Creation Time / QA Time axes + Time/No Time composite toggle.

const RE_VERSION = '13.0';
...
// Tracked-time trapezoid (minutes): full credit between T_LO and T_HI; 0 at 0 and at MAX.
const RE_TIME_T_LO = 15;
const RE_TIME_T_HI = 60;
const RE_TIME_MAX = 150;
const RE_TIME_SHRINK_C = 10;
const RE_TWQS_TIME_ANCHOR_MIN = 35;
const RE_QAQS_TIME_ANCHOR_MIN = 32;
...
const RE_TIER_THRESHOLDS = {
    twqs: {
        flat:         { p10: 64.266, p30: 70.694, p70: 77.183, topMin: 80 },
        recency:      { p10: 70.163, p30: 73.54, p70: 76.489, topMin: 80 },
        flatNoTime:   { p10: 56.38, p30: 64.057, p70: 72.329, topMin: 80 },
        recencyNoTime:{ p10: 63.069, p30: 66.97, p70: 70.646, topMin: 80 },
    },
    qaqs: {
        flat:         { p10: 67.626, p30: 72.736, p70: 78.892, topMin: 78.902 },
        recency:      { p10: 71.43, p30: 75.034, p70: 79.402, topMin: 79.412 },
        flatNoTime:   { p10: 62.77, p30: 67.75, p70: 74.414, topMin: 74.424 },
        recencyNoTime:{ p10: 65.212, p30: 69.644, p70: 74.912, topMin: 74.922 },
    },
};
...
const RE_PERCENTILE_PARAMS = {
    twqs: {
        flat:         { mu: 73.1805, sigma: 7.1256 },
        recency:      { mu: 74.8569, sigma: 3.9782 },
        flatNoTime:   { mu: 67.3506, sigma: 8.2430 },
        recencyNoTime:{ mu: 68.6933, sigma: 4.9307 },
    },
    qaqs: {
        flat:         { mu: 75.4050, sigma: 6.4006 },
        recency:      { mu: 76.7594, sigma: 4.7228 },
        flatNoTime:   { mu: 71.0194, sigma: 6.6713 },
        recencyNoTime:{ mu: 71.7810, sigma: 5.0539 },
    },
};
...
/**
 * Tracked-time trapezoid on average minutes.
 * Full credit between T_LO and T_HI; linear taper to 0 at 0 and at MAX.
 */
function reTimeTrapezoidScore(avgMinutes) {
    const minutes = Number(avgMinutes);
    if (!Number.isFinite(minutes)) return null;
    const tLo = RE_TIME_T_LO;
    const tHi = RE_TIME_T_HI;
    const tMax = RE_TIME_MAX;
    if (minutes < tLo) {
        if (!(tLo > 0)) return 0;
        return Math.max(0, minutes / tLo);
    }
    if (minutes <= tHi) return 1;
    const span = tMax - tHi;
    if (!(span > 0)) return 0;
    return Math.max(0, 1 - (minutes - tHi) / span);
}
...
const RatingEngine = {
    ...
    compute(options) {
        ...
        const workers = authorIds.map((workerId) => {
            ...
            const flatTwqs = this._computeTwqs(workerId, hydratedItems, 'flat', window, nowMs, globalTwCohort, includeTime);
            const recencyTwqs = this._computeTwqs(workerId, hydratedItems, 'recency', window, nowMs, globalTwCohort, includeTime);
            const flatQaqs = this._computeQaqs(workerId, hydratedItems, 'flat', window, nowMs, globalQaCohort, includeTime);
            const recencyQaqs = this._computeQaqs(workerId, hydratedItems, 'recency', window, nowMs, globalQaCohort, includeTime);
        ...
        for (const weighting of ['flat', 'recency']) {
            ...
            for (const dimension of ['team', 'env', 'month']) {
                ...
                for (const [key, sliceItems] of keyedItems) {
                    ...
                    const twBlock = this._computeTwqs(workerId, sliceItems, weighting, window, nowMs, twCohort, includeTime);
                    const qaBlock = this._computeQaqs(workerId, sliceItems, weighting, window, nowMs, qaCohort, includeTime);
    ...
    _computeTwqs(workerId, hydratedItems, weighting, window, nowMs, cohort, includeTime) {
        ...
        // V1 Creation Time: weighted mean of tracked minutes.
        let vtSumWM = 0, vtSumW = 0, vtCount = 0;
        ...
        const vtShrunkMean = vtDefined
            ? reShrunkMeanMinutes(vtSumWM, vtSumW, RE_TIME_SHRINK_C, RE_TWQS_TIME_ANCHOR_MIN)
            : null;
        const vtScore = vtDefined ? reTimeTrapezoidScore(vtShrunkMean) : null;
        ...
        const axes = RE_TWQS_AXES.map((def) => {
            ...
            switch (def.id) {
                ...
                case 'v1CreationTime':
                    ...
                    score = vtScore;
                    raw = {
                        avgMinutes: vtShrunkMean != null ? Math.round(vtShrunkMean * 10) / 10 : null,
                        rawAvgMinutes: vtRawMean != null ? Math.round(vtRawMean * 10) / 10 : null,
                        weightedN: Math.round(vtSumW * 1000) / 1000,
                        thresholdLow: RE_TIME_T_LO,
                        thresholdHigh: RE_TIME_T_HI,
                        thresholdMax: RE_TIME_MAX,
                        anchorMinutes: RE_TWQS_TIME_ANCHOR_MIN,
    ...
    _computeQaqs(workerId, hydratedItems, weighting, window, nowMs, cohort, includeTime) {
        ...
        // QA Time: weighted mean of tracked review minutes.
        let qtSumWM = 0, qtSumW = 0, qtCount = 0;
        ...
        const qtShrunkMean = qtDefined
            ? reShrunkMeanMinutes(qtSumWM, qtSumW, RE_TIME_SHRINK_C, RE_QAQS_TIME_ANCHOR_MIN)
            : null;
        const qtScore = qtDefined ? reTimeTrapezoidScore(qtShrunkMean) : null;
        ...
        const axes = RE_QAQS_AXES.map((def) => {
            ...
            switch (def.id) {
                ...
                case 'qaTime':
                    ...
                    score = qtScore;
                    raw = {
                        avgMinutes: qtShrunkMean != null ? Math.round(qtShrunkMean * 10) / 10 : null,
                        rawAvgMinutes: qtRawMean != null ? Math.round(qtRawMean * 10) / 10 : null,
                        weightedN: Math.round(qtSumW * 1000) / 1000,
                        thresholdLow: RE_TIME_T_LO,
                        thresholdHigh: RE_TIME_T_HI,
                        thresholdMax: RE_TIME_MAX,
                        anchorMinutes: RE_QAQS_TIME_ANCHOR_MIN,
                        shrinkC: RE_TIME_SHRINK_C,
                        observedCounts: {
