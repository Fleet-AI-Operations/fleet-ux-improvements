...
const plugin = {
    ...
    _createModal() {
        ...
        modal.innerHTML = `
            ...
            <!-- Debug Section -->
            <div style="${devPlugins.length > 0 ? 'border-top: 1px solid var(--border, #e5e5e5); padding-top: 16px;' : ''} margin-bottom: 16px;">
                <h3 style="font-size: 14px; font-weight: 600; margin: 0 0 12px 0; color: var(--foreground, #333);">
                    Debug Options
                </h3>
    ...
    _getSettingsTabs(devPlugins = []) {
        const tabs = [
            { id: 'settings', label: 'Settings' }
        ];
        
        // Add Dev tab if on dev branch (dev settings are available)
        if (Context.isDevBranch) {
