...
- **Deleted remote feature branch / no archetypes.json**: with both the feature and main userscripts installed, the feature build probes `archetypes.json` at handshake. HTTP 404 sets a sticky page `localStorage` orphan marker (`fleet-dev-orphan-branch`), clears the dev-active claim, reloads once, and the main userscript runs instead. On later loads the feature build re-probes that marker: if archetypes are back (HTTP 200), the orphan flag is cleared and the feature script runs again. The main install must include this handshake logic (update from `main`). Transient network errors do not orphan a live branch; while a marker is already set, a failed re-probe keeps yielding. You can still remove leftover feature / DEV-ID scripts when convenient.
