...
- **Deleted remote feature branch / no archetypes.json**: with both the feature and main userscripts installed, the feature build probes `archetypes.json` at handshake. HTTP 404 sets a sticky page `localStorage` orphan marker (`fleet-dev-orphan-branch`), clears matching DEV-ID / DEV_ACTIVE claims, reloads once, and the main userscript runs instead. While that orphan marker is set, main always takes over (any leftover feature claims are cleared). Feature re-probe recovery (valid JSON `archetypes.json` HTTP 200) runs only when main is not installed; if main is present the feature script stays yielded. The main install must include this handshake logic (update from `main`). Transient network errors do not orphan a live branch; while a marker is already set and main is absent, a failed re-probe keeps yielding. You can still remove leftover feature / DEV-ID scripts when convenient.
