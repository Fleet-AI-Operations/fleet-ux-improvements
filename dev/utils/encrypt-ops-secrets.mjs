...
//
// Plaintext (gitignored):  local/ops-secrets.json
...
function readPlaintextJson() {
    if (!fs.existsSync(PLAINTEXT_PATH)) {
        throw new Error('Plaintext not found: ' + PLAINTEXT_PATH + ' (copy from ops-secrets.example.json)');
    }
    const text = fs.readFileSync(PLAINTEXT_PATH, 'utf8');
    JSON.parse(text);
    return text;
}
